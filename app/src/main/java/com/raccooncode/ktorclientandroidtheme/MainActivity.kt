...
@Composable
fun Login(service: PostsService) {
    ...
    Column(
        ...
        /*Icon(
            painter = painterResource(id = R.drawable.raccway),
            null,
            Modifier.size(80.dp),
            tint = Color.White
        )*/
        var name = ""
        TextInput(InputType.Name, name, onChange = {name = it})
        var password = ""
        TextInput(InputType.Password, password, onChange = {password = it})
        Button(onClick = {
            Log.d("hello my dog","hi")
            scope.launch {
                val post = PostRequest(title = name, body = password, userId = 1)
                result = service.createPost(post)?.title.toString()
                Log.d("help", result)
                showSnackbar = true
            }
...
@Composable
fun TextInput(inputType: InputType, textData: String, onChange: (String) -> Unit = {}) {
    var value by remember { mutableStateOf(textData) }
